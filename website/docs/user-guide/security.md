...
approvals:
  ...
  cron_mode: deny                 # deny | smart | approve — cron behavior for dangerous commands
...
| `cron_mode` | `deny` | How [cron jobs](./features/cron.md) behave headlessly when they trigger a dangerous-command prompt. `deny` blocks the command (the agent must find another path); `smart` uses the auxiliary LLM approval gate and fails closed on uncertainty; `approve` auto-approves everything in cron context. |
