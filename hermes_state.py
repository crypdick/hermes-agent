        ...
        *,
        ...
        for input_key, column in field_map.items():
            if input_key in context_filter and context_filter[input_key] is not None:
                value = str(context_filter[input_key]).strip()
                if value:
    ...
    def search_messages(
        ...
        if is_cjk:
            ...
            if cjk_count >= 3 and not _any_short_cjk:
                ...
                tri_where.extend(context_clauses)
                tri_params.extend(context_params)
                tri_sql = f"""
                    SELECT
                        m.id,
                        m.session_id,
                ...
                like_where.extend(context_clauses)
                like_params.extend(context_params)
                like_sql = f"""
