...
def _session_env_reader() -> Callable[[str, str], str]:
    try:
        from gateway.session_context import get_session_env as _get_session_env
        return _get_session_env
    except Exception:
        return lambda name, default="": os.getenv(name, default)
...
def _build_context_filter(
    *,
    ...
    """Build a normalized DB context filter from tool args/session context."""
    effective_scope = (scope or "current_context").strip().lower()
    if effective_scope == "all":
        return None
    if effective_scope not in {"current_context", "current_thread"}:
        effective_scope = "current_context"

    get_env = _session_env_reader()
    platform = (platform or get_env("HERMES_SESSION_PLATFORM", "") or "").strip()
    chat_id = (chat_id or get_env("HERMES_SESSION_CHAT_ID", "") or "").strip()
    thread_id = (thread_id or get_env("HERMES_SESSION_THREAD_ID", "") or "").strip()
    routing_key = (routing_key or get_env("HERMES_SESSION_KEY", "") or "").strip()
    user_id = (user_id or "").strip()

    context_filter: Dict[str, Any] = {}
    if platform:
        context_filter["platform"] = platform
    if routing_key and effective_scope == "current_context":
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "(`deploy*`).\n\n"
        "SCOPED RECALL\n\n"
        "  By default, gateway sessions search the current chat/topic context first. "
        "Pass scope='all' only when you explicitly need global search across every session. "
        "You can also pass platform/chat_id/thread_id/routing_key to target a specific context.\n\n"
        "WHEN TO USE\n\n"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "scope": {
                ...
                "description": (
                    "Optional recall scope. Defaults to current_context when gateway "
                    "chat/topic context is available. Use all for explicit global recall."
                ),
            },
            "platform": {
                "type": "string",
            ...
            "chat_id": {
                "type": "string",
                "description": "Optional external chat/channel id filter for scoped recall.",
            },
            "thread_id": {
                "type": "string",
                "description": "Optional external topic/thread id filter for scoped recall.",
            },
            "user_id": {
                "type": "string",
                "description": "Optional sender/user id filter for explicit person-scoped recall.",
            },
            "routing_key": {
                "type": "string",
                "description": "Optional exact persistent gateway session routing key.",
            },
        },
        "required": [],
    },
