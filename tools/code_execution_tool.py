...
# Tools allowed inside the execute_code sandbox. The intersection of this
# list and the session's enabled tools determines which stubs are generated.
#
# Do not expose terminal() here: execute_code already runs inside the tool
# execution path, and nested terminal calls can deadlock the gateway/local
# environment lifecycle. Use the normal terminal tool as a separate tool call.
