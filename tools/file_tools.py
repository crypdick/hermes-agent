...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file. Check this before broad system prefixes so tests and temp-home
    # installs get the more specific error.
...
def _using_local_terminal_backend() -> bool:
    """Return True when file tools can read the host filesystem directly."""
    try:
        from unittest.mock import Mock

        if isinstance(_get_file_ops, Mock):
            return False
    except Exception:
        pass
    try:
...
def _local_similar_files(path: Path) -> list[str]:
...
def patch_tool(
    ...
    if mode == "patch" and patch:
        ...
        for _m in _re.finditer(
            ...
            # ``path=`` arg, so reject traversal in headers.
    ...
    try:
        ...
        with ExitStack() as _locks:
            ...
            if mode == "replace":
                ...
                # operates on the exact file the tool layer resolved.
            ...
            # mismatch is visible in the response instead of silently landing elsewhere.
            _resolved_modified = [
