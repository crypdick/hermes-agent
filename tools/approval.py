...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_gateway = False if is_cron else _is_gateway_approval_context()

    # Cron mode is the unattended-run policy. It must take precedence over any
    # remembered session approval from an earlier interactive surface, otherwise
    # a stale allow can silently defeat approvals.cron_mode: deny.
    if is_cron:
...
def check_all_command_guards(command: str, env_type: str,
    ...
    cron_mode = _get_cron_approval_mode() if is_cron else ""
