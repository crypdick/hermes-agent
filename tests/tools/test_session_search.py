...
class TestBrowseShape:
    ...
    def test_browse_passes_context_filter_when_provided(self):
...
class TestScopedRecall:
    def test_current_context_scope_uses_gateway_session_context(self):
        ...
        try:
            assert _build_context_filter(scope="current_context") == {
                "platform": "telegram",
                "routing_key": "telegram:group:-1001:topic:10",
            }
            assert _build_context_filter(scope="current_thread") == {
                "platform": "telegram",
    ...
    def test_global_scope_disables_context_filter(self):
        assert _build_context_filter(scope="all", platform="telegram") is None

    def test_discovery_passes_explicit_context_filter_to_db_search(self):
        from unittest.mock import MagicMock
